...
Status: `runtime_test.mbt` covers the first runtime slice: wrong-state
transition rejection, role/guard availability, high-value payment branching,
expression precedence, missing actor paths, and GUI field modes.
...
| Camunda 7 gateway quick start | <https://docs.camunda.org/get-started/quick-start/gateway/> |
| Flowable holiday request example | <https://www.flowable.com/open-source/docs/bpmn/ch02-GettingStarted/> |
| JSON Forms rules | <https://jsonforms.io/docs/uischema/rules> |
| JSON Schema Test Suite | <https://github.com/json-schema-org/JSON-Schema-Test-Suite> |

## Selection policy

最優先は current contract / runtime にそのまま落とせるケース。次に
`transition-semantics-v1` / `expression-language-v1` の仕様化に使えるケースを扱う。
BPMN や JSON Schema の完全互換ではなく、業務ルールとして移植できるものだけを残す。
...
| Transitions | `submit` moves `draft` -> `submitted` with `days > 0`; `approve` moves `submitted` -> `approved` for `manager` when `approved == true`; `reject` moves `submitted` -> `rejected` for `manager` when `approved == false` |
| Tests | SQL / normalized / API / GUI manifest golden; runtime true path, false path, missing `approved`, wrong role, wrong state |
...
| Transitions | `submitSmall` moves `draft` -> `autoApproved` when `amount < 1000`; `submitLarge` moves `draft` -> `needsApproval` when `amount >= 1000`; `approve` moves `needsApproval` -> `approved` for `manager`; `reject` moves `needsApproval` -> `rejected` for `manager` with local input `managerComment` |
| Tests | guard true/false matrix; `project_gui` field mode for `draft` vs `needsApproval`; no available transition when `amount` is missing or invalid |
...
| Rules | `counterIsTen` uses `counter == 10`; `nameIsFooOrBar` uses `name == "foo" || name == "bar"`; `needsDetail` uses `needsDetail == true` |
| Tests | rule active/inactive matrix; GUI projection includes rule statuses; rule-driven visibility remains a future spec |
