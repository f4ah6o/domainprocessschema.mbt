...
- `SchemaRuntimeSessionStore` と関連 section 型を追加し、`source`、`selection`、
  `actor`、`record`、`compile`、`runtime`、`graph`、`locale`、`lastAction`
  を保持する形を public API として定義した。
- source 変更、actor 変更、inspector edit、transition 実行で snapshot を再構築する
  library API を追加した。HTTP / UI transport は runtime core に入れていない。
- `export_schema_runtime_session` で stable JSON envelope を出力するようにした。
- focused tests を追加し、resume semantics と transition 後の runtime snapshot 更新を
  検証した。
